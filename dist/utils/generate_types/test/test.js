...
playwright.chromium.launch().then(async (browser) => {
    ...
    await page.exposeFunction('md5', (text) => crypto
        .createHash('md5')
        .update(text)
        .digest('hex'));
...
(async () => {
    ...
    const cookies = await page.context().cookies(['http://example.com']);
    const cookie = cookies[0];
    const nameIsString = true;
    const valueIsString = true;
    const pathIsString = true;
    const expiresIsNumber = true;
    const httpOnlyIsBoolean = true;
    const secureIsBoolean = true;
    const sameSiteIsEnum = true;
...
(async () => {
    ...
    const page = await browser.newPage();
    await page.goto('https://example.com');
    await page.$eval('#someElement', (element, text) => {
    ...
    for (const object of [frame, handle, page]) {
        ...
        {
            const value = await object.$eval('input[foo=bar]', (i) => i.disabled);
            const assertion = true;
        }
        {
        ...
        {
            const value = await object.$eval('input[foo=bar]', (i, dummy) => i.disabled, 2);
            const assertion = true;
        }
        {
        ...
        {
            const value = await object.$$eval('input[foo=bar]', (i) => i[0].defaultValue);
            const assertion = true;
        }
        {
        ...
        {
            const value = await object.$$eval('input[foo=bar]', (i, dummy) => i[0].defaultValue, 2);
            const assertion = true;
        }
    }
    await browser.close();
...
(async () => {
    ...
    {
        const result = await locator.evaluate((sel) => sel.options[sel.selectedIndex].textContent);
        const assertion = true;
    }
    {
        const result = await locator.evaluate((media, dummy) => media.duration, 10);
        const assertion = true;
    }
    {
    ...
    {
        const list = await locator.evaluateAll((i) => i.length);
        const assertion = true;
    }
    {
        const list = await locator.evaluateAll((i, dummy) => i.length, 10);
        const assertion = true;
    }
    {
        await locator.evaluateAll((sel) => { });
    }
    await browser.close();
})();
// waitForEvent
...
(async () => {
    ...
    {
        const value = await windowHandle.evaluate((x, b) => b, 'world');
        const assertion = true;
    }
    {
    ...
    {
        const handle = await page.locator('body').evaluateHandle(() => ([{ a: '123' }]));
        const value = await handle.evaluate((h) => h[1].b);
        const assertion = true;
    }
    {
        const handle = await page.locator('body').evaluateHandle(() => ([{ a: '123' }]));
        const value = await handle.evaluate((h, prop) => h[1][prop], 'b');
        const assertion = true;
    }
    {
        const handle = await page.locator('body').evaluateHandle(() => ([{ a: '123' }]));
    ...
    {
        const handle = await page.locator('body').evaluateHandle(() => ([{ a: '123' }]));
        const value = await handle.evaluateHandle((h) => h[1].b);
        const assertion = true;
    }
    {
        const handle = await page.locator('body').evaluateHandle(e => {
            const assertion1 = true;
            const assertion2 = true;
            return e.nodeName;
        });
    ...
    }
    {
        ...
        const value = await handle.evaluate((e) => e.disabled);
        const assertion = true;
    }
    {
        const handle = await page.waitForSelector('*');
        const value = await handle.evaluate((e, x) => e.disabled || x, 123);
        const assertion = true;
    }
    {
        const handle = await page.waitForSelector('*');
        const value = await handle.evaluateHandle((e, x) => e.disabled || x, 123);
        const assertion = true;
    }
    {
    ...
    {
        const handle = await windowHandle.evaluateHandle((x, b) => b, 123);
        const value = await handle.jsonValue();
        const assertion = true;
    }
    {
...
(async () => {
    ...
    await page.$eval('span', (element, x) => {
        const spanAssertion = true;
        const numberAssertion = true;
    }, 5);
    await page.$eval('my-custom-element', (element, x) => {
        const elementAssertion = true;
        const numberAssertion = true;
    }, 5);
...
(async () => {
    ...
    playwright.errors.TimeoutError;
    {
        ...
        const assertion = true;
        const widthAssertion = true;
        const deviceScaleFactorAssertion = true;
        const hasTouchAssertion = true;
        const isMobileAssertion = true;
    }
    {
